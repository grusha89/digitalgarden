...
In the SEPA payment order, the following new fields are mandatory:

- **City name** – we don’t have this field in wflow, so we fill it with the supplier’s address (max. 35 characters).
    
- **Country code** – we take the first two characters from the supplier’s IBAN.